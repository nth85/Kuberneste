## Thì dùng nhiều trong tiếng anh

Hiện tại đơn | Hien tại Hoàn Thành | Quá khứ đơn | Tương lai đơn
-- | -- | -- | --
Mô tả hệ thống. quy trinh, trang thai hien tai | Bao cao ket qua, thong bao vua xong | Bao cao su co, su viec da xay ra | Ke hoach, Cam ket
-- | -- | -- | --
the system runs automatically | The deployment has been completed | the server crashed last night| I will check the isse
this job is completed | The issue has been fixed | this job was completed yesterday | We will deploy tonight
Logs are stored in Elasticsearch | I have updated the config |

